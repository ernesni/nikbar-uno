...
@Route(value = Constante.PAGE_CONTROL_BENEFICIARIO, layout = MainView.class)
@PageTitle(Constante.TITLE_CONTROL_BENEFICIARIO)
@RolesAllowed({"ADMIN", "SUPERVISOR"})
public class ControlBeneficiarioView extends SplitViewFrame {
	...
	private Grid<ComprobanteBeneficiario> grid;
	...
	public ControlBeneficiarioView(@Autowired ComprobanteBeneficiarioServiceImpl comprobanteBeneficiarioService) {
	...
	private Grid<ComprobanteBeneficiario> createGrid() {
		
		grid = new Grid<>();
		grid.addSelectionListener(event -> event.getFirstSelectedItem().ifPresent(this::showDetails));
		grid.setItems(comprobanteBeneficiarioService.findAllComprobanteBeneficiarioByChequeadoFalse());
		grid.setHeightFull();
		grid.setColumnReorderingAllowed(true);
		grid.addThemeVariants(GridVariant.LUMO_ROW_STRIPES);
		
		grid.addColumn(ComprobanteBeneficiario::getIdcomprobante).setFlexGrow(0).setFrozen(true).setHeader("ID").setSortProperty("idcomprobante").setTextAlign(ColumnTextAlign.END);
		grid.addColumn(ComprobanteBeneficiario::getNumero_comprobante).setHeader("Comprobante N°").setTextAlign(ColumnTextAlign.START).setSortProperty("numero_comprobante");
		grid.addColumn(cb -> cb.getBeneficiario().getFirstNombreFirstApellido()).setHeader("Beneficiario").setTextAlign(ColumnTextAlign.START);
		grid.addColumn(cb -> cb.getGrupo().getNombre()).setHeader("Grupo").setTextAlign(ColumnTextAlign.START).setSortProperty("grupo.nombre");
	
		for (Column<ComprobanteBeneficiario> column : grid.getColumns()) {
	...
	private void showDetails(ComprobanteBeneficiario comprobanteBeneficiario) {
		
		isdeleteImage = false;
		detailsDrawerHeader.setTitle("Comprobante N°: " + comprobanteBeneficiario.getNumero_comprobante());
		detailsDrawer.setContent(createDetails(comprobanteBeneficiario));
	...
	private FormLayout createDetails(ComprobanteBeneficiario comprobanteBeneficiario) {
		...
		numeroField.setValue(comprobanteBeneficiario.getNumero_comprobante());
		...
		image = createFoto(comprobanteBeneficiario);
		isdeleteImage = true;
	...
	private Image createFoto(ComprobanteBeneficiario comprobanteBeneficiario) {
		...
		try {
			...
			if (!isdeleteImage) {
				createContextMenu(foto, comprobanteBeneficiario);
			}
		} catch (IOException e) {
	...
	private void createContextMenu(Image foto, ComprobanteBeneficiario comprobanteBeneficiario) {
		ContextMenu contextMenu = new ContextMenu();
		contextMenu.setTarget(foto);
		contextMenu.addItem("Ver Foto", e -> {
			new VerImagen("Comprobante N°: " + comprobanteBeneficiario.getNumero_comprobante(),
					UIUtils.cargarFotoSinResize(getByte(comprobanteBeneficiario.getUrl()), false));
		});
	}
